...
% Considering departure position is center -> lb=[0.5,0.5]
%quadprog parameters
lb=[0.5,0.5];
ub = ones(size(lb));
Aeq = [1 0];
...
departure_step = step*10;
for i=1:step:departure_step
    ...
    elseif(i>1)
        beq = W(2,(i+step-1)/step -1);
        w=quadprog(H_s,B_s,[],[],Aeq,beq,lb,ub);
    end
    W=[W,w];
...
for i=departure_step+1:step:n-step
    h_s1=(x_l(i+step)-x_r(i+step))^2+(y_l(i+step)-y_r(i+step))^2;
    h_s2=-(x_l(i)-x_r(i))*(x_l(i+step)-x_r(i+step))-(y_l(i)-y_r(i))*(y_l(i+step)-y_r(i+step));
    h_s3=h_s2;
    h_s4=(x_l(i)-x_r(i))^2+(y_l(i)-y_r(i))^2;
